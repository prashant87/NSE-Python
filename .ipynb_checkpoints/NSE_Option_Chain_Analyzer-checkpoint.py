...
class Nse:
    ...
    def get_dataframe(self) -> Optional[Tuple[pandas.DataFrame, str, float]]:
        ...
        df: pandas.DataFrame = pandas.read_json(StringIO(response.text))
        df = df.transpose()
    ...
    def set_values(self) -> None:
        ...
        self.sheet.insert_row(output_values)
        if self.live_export:
    ...
    def main(self) -> None:
        ...
        for i in range(len(entire_oc)):
            int_call_oi: int = int(entire_oc.iloc[i, 0])
            # int_call_oi: int = int(entire_oc.iloc[i])            
        ...
        for i in range(len(entire_oc)):
            int_put_oi: int = int(entire_oc.iloc[i, 20])
        ...
        if self.max_call_oi_sp == self.max_put_oi_sp:
            ...
            for i in range(put_oi_index, call_oi_index):
                int_call_oi_2: int = int(entire_oc.iloc[i, 0])
            ...
            for i in range(put_oi_index + 1, call_oi_index + 1):
                int_put_oi_2: int = int(entire_oc.iloc[i, 20])
